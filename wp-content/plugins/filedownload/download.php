...
// V0.2 Security: not longer allowed relative pathes
if (strpos($path, 'http://') === false) exit();
if (!(strpos($path, '..') === false)) exit();
if (!(strpos($path, '.php') === false)) exit();
...
if ($type == "") $type = filedownload_Mime($extension);
...
if($open)
{
  ...
  //filedownload_CreateTables();
  filedownload_Write($path);
  // start download
...
function filedownload_Mime($ext) 
{
